...
class CTTorrentGenerator:
    ...
    def _create_torrent_for_range(
        ...
        for level, paths in tile_paths.items():
            ...
            for tile_path in paths:
                ...
                # Copy the file
                with open(tile_path, "rb") as src, open(
                    target_dir / rel_path.name, "wb"
                ) as dst:
                    dst.write(src.read())
        ...
        if issuer_paths:
            ...
            for issuer_path in issuer_paths:
                ...
                with open(issuer_path, "rb") as src, open(
                    issuer_dir / rel_path, "wb"
                ) as dst:
                    dst.write(src.read())
    ...
    def _create_torrent(
        self, content_dir: Path, torrent_name: str, comment: str
    ) -> str:
        """
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate torrent files for a Static CT log mirror"
    )
    ...
    parser.add_argument(
        ...
        "-o",
        default="./torrents",
        help="Directory to save generated torrent files",
    )
    parser.add_argument(
        "--entries-per-torrent",
        "-e",
    ...
    parser.add_argument(
        "--private", action="store_true", help="Set the private flag in the torrent"
    )
    ...
    config = TorrentConfig(
        piece_length=args.piece_length, announce=args.announce, private=args.private
    )
